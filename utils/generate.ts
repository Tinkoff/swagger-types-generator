...
function writeFile(path, contents) {
  mkdir(dirname(path), (err) => {
    if (err) {
      ...
      console.log(`[${chalk.yellow('WARNING')}] Can't mkdir for ${dirname(path)} ${err}`);
    }
    try {
      writeFileSync(path, autogeneratedHeaders(contents));
      console.log(`[${chalk.green('SUCCESS')}] Types (${path}) have been successfully written. 🎉`);
    } catch (e) {
      console.log(`[${chalk.red('ERROR')}] Can't write file for ${path} ${e}`);
    }
  });
}
